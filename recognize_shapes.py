...
common_path = '/Users/indraislas/Desktop/Mine/ll/Firstproject/Pictures/Detected/Third'
...
def process_img(img):
    ...
    canny = cv.Canny(img, 80, 250)
    cv.imshow('Canny', canny)
    cv.imwrite(os.path.join(common_path + '/canny.png'), canny)
    cv.waitKey(0)
    # Apply Dilation
    dilated = cv.dilate(canny, (20, 20), iterations=3)
    cv.imshow('Dilated', dilated)
    cv.imwrite(os.path.join(common_path + '/dilated.png'), dilated)
    cv.waitKey(0)
    # Apply Erosion
    ...
    cv.imwrite(os.path.join(common_path + '/inverted.png'), bgr_img)
    cv.waitKey(0)
    return bgr_img

def get_contours(image, original_img):
    ...
    cv.imshow("Mask", shapeMask)
    cv.imwrite(os.path.join(common_path + '/mask.png'), shapeMask)
    area_thresh = 1000
    ...
    for c in cnts:
        ...
        cv.imwrite(os.path.join(common_path + '/image.png'), image)
        cv.imshow("Original Image", original_img)
        cv.imwrite(os.path.join(common_path + '/original_img.png'), original_img)
        cv.waitKey(0)
    return cnts
...
if __name__ == '__main__':
    ...
    cnts = get_contours(processed_img, original_img.copy())
